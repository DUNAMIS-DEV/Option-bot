 ...
 * ===============================
 * Shows actual backend error messages for debugging.
...
async function generateSetup(symbol) {
  ...
  try {
    ...
    // Check if backend returned an error response (even with 200)
    if (setup.error) {
    ...
    showToast(`Setup failed: ${msg}${hint ? ' — ' + hint : ''}`, 'error', 6000);
