/**
 * app.js — XAU/USD SCALPER + API USAGE (DEBUG EDITION)
 * =======================================================
 * User journey traced & fixed:
 * 1. Page loads → health check, status, market data, trades, usage
 * 2. User sees XAU/USD price (or "Market Closed" on Sunday)
 * 3. User clicks "Generate Setup" → setup fetched and displayed
 * 4. User clicks "Execute Trade" → position opened
 * 5. User clicks "Close" → position closed
 * 6. API Usage bar updates live from response headers
 */
...
const state = {
  ...
  isScanning: false,
  apiUsage: null,
  marketStatus: 'unknown' // 'open', 'closed', 'error', 'unknown'
};
...
async function apiGet(endpoint) {
  ...
  try {
    ...
    console.error(`[FRONTEND] Invalid JSON from ${endpoint}:`, e.message);
    data = { error: 'Invalid JSON response from server', detail: 'Server may have returned HTML instead of JSON' };
  }

  if (!res.ok) {
    const errMsg = data.detail || data.error || data.message || `HTTP ${res.status}`;
    console.error(`[FRONTEND] ${endpoint} failed:`, errMsg);
...
async function checkHealth() {
  try {
    ...
    if (!data.servicesLoaded) {
      showToast('Backend services failed to load. Check server logs.', 'error', 10000);
    }
    return true;
  } catch (err) {
...
// ═══════════════════════════════════════════════════════════════
// PAIRS GRID (Single XAU/USD card)
// ═══════════════════════════════════════════════════════════════
...
function showMarketStatus(sym, status, message) {
  ...
  if (changeEl) {
    changeEl.textContent = status === 'closed' ? 'Sunday — opens 6pm EST' : message;
  ...
  if (lastSigEl) {
    lastSigEl.textContent = status === 'closed' ? 'Markets closed — setups unavailable' : message;
...
async function generateSetup(symbol) {
  ...
  try {
    ...
    // Check for backend-reported errors (200 status but error field)
    if (setup.error) {
...
function handleApiError(err, context) {
  ...
  if (status === 404) {
    showToast('Server endpoint not found. Did you deploy the new server.js?', 'error', 8000);
    ...
    showToast('Server returned invalid data. Check if server.js was updated.', 'error', 8000);
  } else {
...
// ═══════════════════════════════════════════════════════════════
// API USAGE (now works purely from tracked headers)
// ═══════════════════════════════════════════════════════════════
...
function renderApiUsage(data) {
  ...
  // If no data yet, show friendly message
...
function renderApiUsageError(msg) {
  ...
  els.creditBarText.textContent = msg.includes('404') ? 'Server endpoint missing. Deploy new server.js.' : 'Unable to load usage';
  els.creditBarFill.style.width = '0%';
}
