/**
 * app.js — RATE LIMIT PROTECTION
 * ==============================
 * Slower refresh rates, better error messages for Twelve Data free tier.
 * Watchlist: XAU/USD, USD/JPY, GBP/JPY, EUR/USD, GBP/USD
 */

const API_BASE = '';

const PAIRS = ['XAU/USD', 'USD/JPY', 'GBP/JPY', 'EUR/USD', 'GBP/USD'];
...
async function checkHealth() {
  try {
    ...
    if (!data.apiKeyPresent) {
      showToast('TWELVE_DATA_API_KEY not set! Add it to Render Environment Variables.', 'error', 8000);
    }
    return true;
  } catch (err) {
  ...
  try {
    ...
    if (data.allPairs) {
      PAIRS.forEach(sym => {
        ...
        } else if (pairData && pairData.quote && pairData.quote.error) {
          console.error(`[${sym}] Quote error:`, pairData.quote.error);
        }
      });
    }
  } catch (err) {
    console.error('All pairs market data error:', err.message);
  }
}
...
function updatePairCard(sym, quote) {
  ...
  if (priceEl) {
    const decimals = sym === 'XAU/USD' ? 2 : 3;
...
async function generateSetup(symbol) {
  ...
  try {
    ...
    if (setup.error) {
      throw new Error(setup.detail || setup.error || 'Unknown backend error');
    }
    ...
    const hint = err.response?.hint || '';

    if (err.status === 429 || msg.includes('429') || msg.includes('rate')) {
      showToast('Rate limited by Twelve Data. Free tier allows ~8 requests/min. Wait a moment and try again.', 'warning', 6000);
    } else if (err.status === 503) {
      showToast(`API Error: ${msg}${hint ? ' — ' + hint : ''}`, 'error', 6000);
    } else {
      showToast(`Setup failed: ${msg}`, 'error', 6000);
    }
...
async function scanAllPairs() {
  ...
  try {
    ...
    if (err.status === 429 || msg.includes('429') || msg.includes('rate')) {
      showToast('Rate limited by Twelve Data. Free tier allows ~8 requests/min. Wait 30 seconds and try again.', 'warning', 8000);
    } else {
      showToast(`Scan failed: ${msg}${hint ? ' — ' + hint : ''}`, 'error', 6000);
    }
...
function renderSetup(setup) {
  ...
  const decimals = setup.symbol === 'XAU/USD' ? 2 : 3;
  ...
  try {
    ...
    if (data.activePositions && data.activePositions.length > 0) {
      els.tradesBody.innerHTML = data.activePositions.map(pos => {
        const decimals = pos.symbol === 'XAU/USD' ? 2 : 3;
        return `
        <tr>
          <td>${pos.id}</td>
          <td>${pos.symbol}</td>
    ...
    if (data.tradeHistory && data.tradeHistory.length > 0) {
      els.historyBody.innerHTML = data.tradeHistory.map(t => {
        const decimals = t.symbol === 'XAU/USD' ? 2 : 3;
        return `
        <tr>
          <td>${t.id}</td>
          <td>${t.symbol}</td>
...
async function init() {
  ...
  // SLOWER refresh to avoid rate limits (30s instead of 10s)
